...
<form name="f" action="userhome.php" method="post" onsubmit="return check_regn()" style="float:left;">
    <table class="login">
        ...
        <tbody>
            ...
            <tr>
                <th>City</th>
                <td><input type="text" name="city" value="<?php echo $row[2]?>" required></td>
            </tr>
            <tr>
                <th>Gender</th>
            ...
            <tr>
                <th>DOB</th>
                <td><input type="date" name="dob" required value="<?php echo $row[4]?>"></td>
            </tr>
            <tr>
                <th>Mobile</th>
                <td><input type="text" name="mobile" value="<?php echo $row[5]?>" maxlength="10" required></td>
            </tr>
            <tr>
                <th>E-Mail (Userid)</th>
                <td><input type="text" name="email" value="<?php echo $row[6]?>" required readonly></td>
            </tr>
            <tr>
                <th>Password</th>
                <td><input type="password" name="pwd" value="<?php echo $row[7]?>" required></td>
            </tr>
            <tr>
                <th>Confirm Pwd</th>
                <td><input type="password" name="cpwd" value="<?php echo $row[7]?>" required></td>
            </tr>
        </tbody>
        <tfoot>
            <tr>
...
<?php
    ...
    extract($_POST);
    mysqli_query($link, "update newuser set uname='$uname',addr='$addr',city='$city',gender='$gender',dob='$dob',mobile='$mobile',pwd='$pwd' where userid='$email'") or die(mysqli_error($link));
